...
[//run]: # (pandoc azure_tests.rmd -o azure_tests.pdf && open azure_tests.pdf)
...
```
az aks show --resource-group ${resourceGroup} --name ${clusterName} --query nodeResourceGroup -o tsv
...
```(bash)
az network public-ip show --resource-group ${resourceGroupName} --name ${ipName} --query ipAddress --output tsv
```
...
```(bash)
kubectl autoscale deployment ${deployment name} --cpu-percent=${autoescaleTrigger} --min=${minimumPods} --max=${maximumPods}
```
...
```(bash)
az aks scale --resource-group ${resourceGroup} --name ${clusterName} --node-count 2
kubectl autoscale deployment ${deployment name} --cpu-percent=${autoescaleTrigger} --min=2 --max=5
```
...
```(bash)
while true; do wget http://${ipAddress}; done
```
...
Finalmente desactivamos las peticiones pues el ordenador en el que se hizo el test no podía aguantar realizar más sin llegar al máximo de pods que pudiera haber habido.
